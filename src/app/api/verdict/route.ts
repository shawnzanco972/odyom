...
export async function POST(req: NextRequest): Promise<NextResponse<VerdictResponse | { error: string }>> {
  ...
  if (!dev) {
    ...
    newStreak = isDeath ? 0 : userRow.current_streak + 1;
    newScore = userRow.total_score + (isDeath ? 0 : 10 + (32 - state.ballsDropped));
    ...
    // Only snapshot when we actually lose progress — preserves the last
    // meaningful value across multiple deaths in a row.
